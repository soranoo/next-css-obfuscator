...
const pluginName = "Patched PostCSS Obfuscator";
...
`,
        ...
        //! Patch - Start
        const jsonPath = path.join(process.cwd(), jsonsPath, 'main.json');
        const isJsonExist = fs.existsSync(jsonPath);
        const oldJsonData = isJsonExist ? JSON.parse(fs.readFileSync(jsonPath, 'utf8')) : {};

        function checkIfExistInJsonData(className) {
          if (fresh || !oldJsonData) return false;
          let exist = false;
          let outPutClassName = "";
          Object.keys(oldJsonData).forEach(key => {
            if (key.slice(1).replace("\\\\", "\\") == className) {
              exist = true;
              outPutClassName = oldJsonData[key].slice(1); // remove dot at the beginning
            }
          });
          return exist ? outPutClassName : false;
        }
        //! Patch - End
        root.walkRules((rule) => {
          rule.selectors = rule.selectors.map((selector) => {
            ...
            classList.forEach((className) => {
              ...
              if (classIgnore.includes(className) || classMethod == "none") {
                ...
                //! Patch - Start
                newClassName = checkIfExistInJsonData(className) || getRandomName(length);
                //! Patch - End
              }
