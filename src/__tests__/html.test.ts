...
describe("obfuscateHtmlClassNames", () => {
  ...
  it("should handle nested tags with obfuscate class", () => {
    ...
    // Act
    const result = obfuscateHtmlClassNames({
      html,
      selectorConversion,
      obfuscateMarkerClass: keyClass,
    });

    // Assert
    expect(result.obfuscatedContent).toEqual(
      `<div class="key"><span class="a"></span><span class="a"></span></div>`,
    );
    expect(result.usedKeys).to.deep.equal(["\\.foo"]);
  });
  ...
  it("should handle script tags", () => {
    ...
    // Assert
    expect(result.obfuscatedContent).toEqual(
      `<script>self.__next_f.push({\\"className\\":\\"a b\\",})</script>`,
    );
  ...
  it("should handle comments", () => {
    ...
    // Assert
    expect(result.obfuscatedContent).toEqual(
      `<!-- This is a comment --><div class="a"></div>`,
    );
    expect(result.usedKeys).to.deep.equal(["\\.foo"]);
  });
  ...
  it("should handle HTML with nested structures and multiple classes", () => {
    ...
    // Assert
    expect(result.obfuscatedContent).toEqual(
      `<div class="a"><span class="b"><i class="c"></i></span></div>`,
    );
    expect(result.usedKeys).to.deep.equal(["\\.foo", "\\.bar", "\\.baz"]);
  });
  ...
  it("should handle double quot inside double quot", () => {
    ...
    // Assert
    expect(result.obfuscatedContent).toEqual(
      `<div data-opts="{&quot;name&quot;:&quot;MyReactComponent&quot;,&quot;value&quot;:true}" class="a"></div>`,
    );
    expect(result.usedKeys).to.deep.equal(["\\.foo"]);
  });
});
