...
const searchComponent = (content: string, componentName: string) => {
  const componentSearchRegex = new RegExp(
    `\\b(?:const|let|var)\\s+(${componentName})\\s*=\\s*.*?(\\{)`,
    "g",
  );
  ...
  const closeMarkerPos = findClosestSymbolPosition(
    content,
    "{",
    "}",
    openSymbolPos,
...
export const obfuscateForwardComponentJs = (
  searchContent: string,
  wholeContent: string,
  selectorConversion: SelectorConversion,
) => {
  const componentNames = searchForwardComponent(searchContent).filter(
    (componentName) => {
  ...
  const componentsObfuscatedCode = componentsCode.map((componentContent) => {
    const classNameBlocks = findContentBetweenMarker(
    ...
    if (classNameBlocks.length !== obfuscatedClassNameBlocks.length) {
      log(
        ...
        "classNameBlocks.length !== obfuscatedClassNameBlocks.length",
      );
      return componentContent;
    }
    let obscuredCode = componentContent.code;
    for (let i = 0; i < classNameBlocks.length; i++) {
      obscuredCode = replaceFirstMatch(
        obscuredCode,
        classNameBlocks[i],
        obfuscatedClassNameBlocks[i],
      );
    }
...
export const obfuscateJs = (
  content: string,
  key: string,
  selectorCoversion: SelectorConversion,
  filePath: string,
  ...
  if (useAst) {
    try {
      const { obfuscatedCode, usedKeys } = obfuscateJsWithAst(
        content,
        selectorCoversion,
        key ? [key] : [],
        true,
      ...
      if (content !== obfuscatedCode) {
        log(
          "debug",
          `Obscured keys with AST and marker "${key}":`,
      ...
      if (error instanceof SyntaxError) {
        ...
        log(
          "warn",
          "Obfuscation with AST failed:",
          "Falling back to regex obfuscation",
        );
      } else {
